...
/// Widget screen that lets the doctor log out and returns to role selection.
class DoctorLogoutScreen extends StatefulWidget {
  const DoctorLogoutScreen({super.key});

  @override
  State<DoctorLogoutScreen> createState() => _DoctorLogoutScreenState();
}

class _DoctorLogoutScreenState extends State<DoctorLogoutScreen> {
  bool _loading = false;

  Future<void> _performLogout() async {
    setState(() => _loading = true);
    try {
      await DoctorLogout().execute(context);
      // DoctorLogout navigates away; no further action needed here.
    } catch (_) {
...
class DoctorLogout {
  ...
  DoctorLogout([AuthService? auth]) : _auth = auth ?? AuthService();
  ...
  /// Convenience static method for callers.
  static Future<void> logoutAndGoToRoleSelection(BuildContext context) {
    return DoctorLogout().execute(context);
  }
}